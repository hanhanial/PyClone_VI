...
{
  driver_list_dir = "/mnt/projects/lailhh/workspace/pipelines/PyClone/PyClone_VI/reference/driver-mutation_lists/"
  
  # Use known drivers list
  hccdrivers = read.table(paste0(driver_list_dir,"concensus_1349_koreanindels.txt"),stringsAsFactors = F) 
  tcgadrivers = read_tsv(paste0(driver_list_dir,"tcga_2018_driver_list.txt"), col_types = cols()) %>%
    filter(Cancer %in% c("LIHC", "PANCAN"))
  cgcdrivers = read_csv(paste0(driver_list_dir,"cancer_gene_census.csv"), col_types=cols()) 
  ...
  # signalling pathway
  signalling = read_tsv(paste0(driver_list_dir,"TCGA_2018_signalling_pathways.txt"), col_types=cols())
  ...
  cancermine = read_tsv(paste0(driver_list_dir,"cancermine_collated_2018-10-17.tsv"), col_types=cols()) %>%
